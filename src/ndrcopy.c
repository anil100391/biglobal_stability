...
//    printf("proc %d is responsible for row %d to %d\n",rank,Istart,Iend);
    ...
    for (Ii=Istart; Ii<Iend; Ii++) {
        ...
        case 4:
            ...
            d1dx((int)locIi,nx,ny,&(arg->trnfm),derx);
            for (i=0; i<(nx+1)*(ny+1); i++)
                derx[i] = -(arg->gamma-1)*rho*rho*derx[i];
            derx[locIi] = derx[locIi] + rho*arg->drhodx[locIi];
            j = 0;
            shift = locIi/(ny+1);
            for (i=0; i<(nx+1); i++) {
            ...
            d1dy((int)locIi,nx,ny,&(arg->trnfm),dery);
            for (i=0; i<(nx+1)*(ny+1); i++)
                dery[i] = -(arg->gamma-1)*rho*rho*dery[i];
            dery[locIi] = dery[locIi] + rho*arg->drhody[locIi];
            for (i=0; i<(ny+1); i++) {
                col[j] = (nx+1)*(ny+1) + shift*(ny+1) + i;
                val[j] = dery[col[j]-(nx+1)*(ny+1)];
            ...
            ////////////// energy eqn block 3 ///////////////
            col[j] = 2*(nx+1)*(ny+1) + locIi;
            val[j] = -(arg->gamma-1)*rho*rho*(*beta)*I;
            j = j + 1;
            ////////////// energy eqn block 4 ///////////////
            d1dx((int)locIi,nx,ny,&(arg->trnfm),derx);
            for (i=0; i<(nx+1)*(ny+1); i++) 
            ...
            for(i=0; i<(nx+1)*(ny+1); i++)
                derx[i] = derx[i] + dery[i];
            derx[locIi] = derx[locIi] + (*beta)*rho*w*I + 2*arg->drhodx[locIi]*u + 2*arg->drhody[locIi]*v;
            derx[locIi] = derx[locIi] - 3*(arg->gamma-1)*rho*(arg->dudx[locIi]+arg->dvdy[locIi]);
...
//    printf("proc %d is responsible for row %d to %d\n",rank,Istart,Iend);
    for (Ii=Istart; Ii<Iend; Ii++) {
        ...
        case 4:
            i = Ii-(nx+1)*(ny+1);
            ierr = MatSetValues(arg->B,1,&Ii,1,&i,&one,INSERT_VALUES); CHKERRQ(ierr);
            i = Ii;
